...
class YoloRunner(QThread):
    ...
    def __init__(self, predicted_frame = [], bounding_boxes = [], config_path = [], raw_coordinates = [], filelist = []):
        super().__init__()
        ...
        if isinstance(self.filelist, str):  # 문자열이면 리스트로 변환
    ...
    def run(self):
        for path in self.filelist:
            ...
            self.output = []
            if len(self.filelist) == 1 and self.raw_coordinates:
                self.output = self.raw_coordinates
