 ...
 "cells": [
  {
   "cell_type": "code",
   "execution_count": 170,
   "id": "8a7f240b-ce2f-4c2c-9ed9-ee559eae6747",
   "metadata": {},
   "outputs": [
    {
     "name": "stderr",
     "output_type": "stream",
     "text": [
      "C:\\Users\\Dr. Justin\\AppData\\Local\\Temp\\ipykernel_31460\\1224857335.py:16: DeprecationWarning: sipPyTypeDict() is deprecated, the extension module should use sipPyTypeDictRef() instead\n",
   ...
   "source": [
    ...
    "    def __init__(self):\n",
    "        super().__init__()\n",
    "        self.setWindowTitle(\"Video labeler by Sunpil Kim\")\n",
    ...
    "        self.save_box_button = QPushButton(\"💾 Save BBox\")\n",
    ...
    "        self.save_box_button.clicked.connect(self.save_current_frame_bbox)\n",
    ...
    "    #             # h, w, _ = frame.shape\n",
    "    #             # # scale = 1000 / w if w > 1000 else 1\n",
    "    #             # # self.scaled_w, self.scaled_h = int(w * scale), int(h * scale)\n",
    "    #             # # frame = cv2.resize(frame, (self.scaled_w, self.scaled_h))\n",
    "    #             # frame = cv2.resize(frame, (int(w * self.zoom_scale), int(h * self.zoom_scale)))\n",
    "\n",
    "    #             frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)\n",
    "    #             h, w, _ = frame.shape\n",
    ...
    "    #             if self.start_point and self.end_point and self.is_drawing == True:\n",
    "    #                 cv2.rectangle(frame, self.start_point, self.end_point, (0, 255, 0), 2)\n",
    "                    \n",
    "    #             # Saved Bounding Box\n",
    "    #             for bbox in self.bounding_boxes:\n",
    "    #                 start_x, start_y, end_x, end_y, frame_id, class_id = bbox\n",
    "    #                 if frame_id == int(self.cap.get(cv2.CAP_PROP_POS_FRAMES)):\n",
    "    #                     color = self.class_colors[class_id % len(self.class_colors)]\n",
    "    #                     cv2.rectangle(frame, (start_x, start_y), (end_x, end_y), color, 2)\n",
    "                \n",
    "    #             # q_image = QImage(frame.data, self.scaled_w, self.scaled_h, self.scaled_w * 3, QImage.Format_RGB888)\n",
    "    #             q_image = QImage(frame.data, frame.shape[1], frame.shape[0], frame.strides[0], QImage.Format_RGB888)\n",
    "    #             self.video_label.setPixmap(QPixmap.fromImage(q_image))\n",
    "                \n",
    "    #             self.video_label.setFixedSize(scaled_w, scaled_h)\n",
    "\n",
    "    #             self.playback_slider.setValue(int(self.cap.get(cv2.CAP_PROP_POS_FRAMES)))\n",
    "    #             self.update_frame_label()\n",
    "    #         else:\n",
    "    #             self.is_playing = False\n",
    "    \n",
    ...
    "                )\n",
    "    \n",
    "    def save_current_frame_bbox(self):\n",
    "        if not self.cap:\n",
    "            print(\"[Error] No video loaded.\")\n",
    "            return\n",
    "\n",
    "        save_dir = \"./result/\" + self.current_filename\n",
    "        os.makedirs(save_dir, exist_ok=True)\n",
    "        ret, frame = self.cap.read()\n",
    "        if ret:\n",
    "            image_path = f\"{save_dir}/frame_{self.current_frame:06d}.png\"\n",
    "            cv2.imwrite(image_path, frame)\n",
    "            print(f\"[Info] Frame saved at: {image_path}\")\n",
    "        else:\n",
    "            print(\"[Error] Failed to read the frame.\")\n",
    "            return\n",
    "\n",
    "        bbox_path = f\"{save_dir}/frame_{self.current_frame:06d}.txt\"\n",
    ...
    "            print(f\"[Info] Bounding boxes saved at: {bbox_path}\")\n",
    "\n",
    "    def save_all_frames_and_bboxes(self):\n",
    "        \"\"\"모든 Bounding Box와 프레임 이미지를 저장합니다.\"\"\"\n",
    "        if not self.cap:\n",
    "            print(\"[Error] No video loaded.\")\n",
    "            return\n",
    "        \n",
    ...
    "        os.makedirs(save_dir, exist_ok=True)\n",
    "        \n",
    "        for frame_id, bboxes in self.bounding_boxes.items():\n",
    "            # 프레임 위치로 이동\n",
    ...
    "            \n",
    "            # 프레임 이미지 저장\n",
    "            image_path = f\"{save_dir}/frame_{frame_id:06d}.png\"\n",
    "            cv2.imwrite(image_path, frame)\n",
    "            print(f\"[Info] Frame saved at: {image_path}\")\n",
    "            \n",
    "            # Bounding Box 저장\n",
    "            bbox_path = f\"{save_dir}/frame_{frame_id:06d}.txt\"\n",
    "            with open(bbox_path, \"w\") as f:\n",
    "                for bbox in bboxes:\n",
    "                    x1, y1, x2, y2, class_id = bbox\n",
    "                    \n",
    ...
    "            print(f\"[Info] Bounding boxes saved at: {bbox_path}\")\n",
    "        \n",
    "        print(\"[Success] All frames and bounding boxes have been saved!\")\n",
    "    \n",
  ...
  },
  {
   ...
   "id": "f9dd8aa7-5ce3-4bf5-831a-bb07e92e1b7f",
   "metadata": {},
   "outputs": [],
   "source": []
  }
